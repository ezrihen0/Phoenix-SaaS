  ...
  private async ensureRecentCallsSchema() {
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS recent_calls (
        ...
        from_number_normalized varchar(32) NULL,
        to_number varchar(64) NULL,
        to_number_normalized varchar(32) NULL,
        source varchar(64) NOT NULL DEFAULT 'unknown',
        source_mapping_id varchar(128) NULL,
        campaign_name varchar(255) NULL,
        inbound_owned_phone_number_id char(36) NULL,
        inbound_owned_phone_number_label varchar(255) NULL,
        inbound_market_key varchar(64) NULL,
        inbound_market_label varchar(128) NULL,
        call_status varchar(64) NOT NULL,
        processing_status varchar(64) NOT NULL,
        business_hours_status varchar(32) NULL,
        call_flow_action varchar(64) NULL,
        call_flow_route_target varchar(255) NULL,
        ivr_status varchar(64) NULL,
        route_execution_status varchar(64) NULL,
        route_execution_detail longtext NULL,
        route_command_id varchar(255) NULL,
        whisper_text longtext NULL,
        whisper_status varchar(64) NULL,
        whisper_command_id varchar(255) NULL,
        voicemail_url longtext NULL,
        voicemail_status varchar(64) NULL,
        provider_voicemail_recording_id varchar(255) NULL,
        queue_status varchar(64) NULL,
        queue_position int NULL,
        queue_entered_at datetime(6) NULL,
        queue_exited_at datetime(6) NULL,
        queue_wait_seconds int NULL,
        queue_callback_requested tinyint(1) NOT NULL DEFAULT 0,
        voicemail_transcription longtext NULL,
        ai_summary longtext NULL,
        ai_sentiment varchar(64) NULL,
        ai_status varchar(64) NULL,
        ai_provider varchar(64) NULL,
        ai_model varchar(128) NULL,
        ai_enriched_at datetime(6) NULL,
        selected_service_type varchar(64) NULL,
        selected_ivr_digit varchar(32) NULL,
        matched_client_id char(36) NULL,
        matched_lead_id char(36) NULL,
        matched_client_display_name varchar(255) NULL,
        recording_url longtext NULL,
        provider_recording_id varchar(255) NULL,
        recording_status varchar(64) NULL,
        duration_seconds int NULL,
        call_started_at datetime(6) NULL,
        call_answered_at datetime(6) NULL,
        call_ended_at datetime(6) NULL,
        raw_payload_snapshot longtext NOT NULL,
        created_at datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
        updated_at datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
        PRIMARY KEY (id),
        UNIQUE KEY ux_recent_calls_provider_event_id (provider, provider_event_id),
        KEY ix_recent_calls_created_at (created_at),
        KEY ix_recent_calls_from_number_normalized (from_number_normalized),
        KEY ix_recent_calls_to_number_normalized (to_number_normalized)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
    `);

    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS missed_call_sms_settings (
        id char(36) NOT NULL,
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS missed_call_sms_cooldowns (
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS recent_call_sms_logs (
        id char(36) NOT NULL,
    ...
    await this.dataSource.query(`
      CREATE TABLE IF NOT EXISTS recent_call_activity_events (
        id char(36) NOT NULL,
    ...
    await this.dataSource.query(`
      ALTER TABLE recent_call_sms_logs
      MODIFY COLUMN recent_call_id char(36) NULL;
    `);

    await this.ensureRecentCallSmsLogsColumn(
      "customer_id",
      "ALTER TABLE recent_call_sms_logs ADD COLUMN customer_id char(36) NULL AFTER recent_call_id",
    );
    await this.ensureRecentCallSmsLogsColumn(
      "read_at",
      "ALTER TABLE recent_call_sms_logs ADD COLUMN read_at datetime(6) NULL AFTER sent_at",
    );
    await this.ensureRecentCallsPrimaryKeyColumn();
    await this.ensureRecentCallsColumn(
      "business_hours_status",
      "ALTER TABLE recent_calls ADD COLUMN business_hours_status varchar(32) NULL AFTER processing_status",
    );
    await this.ensureRecentCallsColumn(
      "inbound_owned_phone_number_id",
      "ALTER TABLE recent_calls ADD COLUMN inbound_owned_phone_number_id char(36) NULL AFTER campaign_name",
    );
    await this.ensureRecentCallsColumn(
      "inbound_owned_phone_number_label",
      "ALTER TABLE recent_calls ADD COLUMN inbound_owned_phone_number_label varchar(255) NULL AFTER inbound_owned_phone_number_id",
    );
    await this.ensureRecentCallsColumn(
      "inbound_market_key",
      "ALTER TABLE recent_calls ADD COLUMN inbound_market_key varchar(64) NULL AFTER inbound_owned_phone_number_label",
    );
    await this.ensureRecentCallsColumn(
      "inbound_market_label",
      "ALTER TABLE recent_calls ADD COLUMN inbound_market_label varchar(128) NULL AFTER inbound_market_key",
    );
    await this.ensureRecentCallsColumn(
      "call_flow_action",
      "ALTER TABLE recent_calls ADD COLUMN call_flow_action varchar(64) NULL AFTER business_hours_status",
    );
    await this.ensureRecentCallsColumn(
      "call_flow_route_target",
      "ALTER TABLE recent_calls ADD COLUMN call_flow_route_target varchar(255) NULL AFTER call_flow_action",
    );
    await this.ensureRecentCallsColumn(
      "ivr_status",
      "ALTER TABLE recent_calls ADD COLUMN ivr_status varchar(64) NULL AFTER call_flow_route_target",
    );
    await this.ensureRecentCallsColumn(
      "route_execution_status",
      "ALTER TABLE recent_calls ADD COLUMN route_execution_status varchar(64) NULL AFTER ivr_status",
    );
    await this.ensureRecentCallsColumn(
      "route_execution_detail",
      "ALTER TABLE recent_calls ADD COLUMN route_execution_detail longtext NULL AFTER route_execution_status",
    );
    await this.ensureRecentCallsColumn(
      "route_command_id",
      "ALTER TABLE recent_calls ADD COLUMN route_command_id varchar(255) NULL AFTER route_execution_detail",
    );
    await this.ensureRecentCallsColumn(
      "whisper_text",
      "ALTER TABLE recent_calls ADD COLUMN whisper_text longtext NULL AFTER route_command_id",
    );
    await this.ensureRecentCallsColumn(
      "whisper_status",
      "ALTER TABLE recent_calls ADD COLUMN whisper_status varchar(64) NULL AFTER whisper_text",
    );
    await this.ensureRecentCallsColumn(
      "whisper_command_id",
      "ALTER TABLE recent_calls ADD COLUMN whisper_command_id varchar(255) NULL AFTER whisper_status",
    );
    await this.ensureRecentCallsColumn(
      "voicemail_url",
      "ALTER TABLE recent_calls ADD COLUMN voicemail_url longtext NULL AFTER whisper_command_id",
    );
    await this.ensureRecentCallsColumn(
      "voicemail_status",
      "ALTER TABLE recent_calls ADD COLUMN voicemail_status varchar(64) NULL AFTER voicemail_url",
    );
    await this.ensureRecentCallsColumn(
      "provider_voicemail_recording_id",
      "ALTER TABLE recent_calls ADD COLUMN provider_voicemail_recording_id varchar(255) NULL AFTER voicemail_status",
    );
    await this.ensureRecentCallsColumn(
      "queue_status",
      "ALTER TABLE recent_calls ADD COLUMN queue_status varchar(64) NULL AFTER provider_voicemail_recording_id",
    );
    await this.ensureRecentCallsColumn(
      "queue_position",
      "ALTER TABLE recent_calls ADD COLUMN queue_position int NULL AFTER queue_status",
    );
    await this.ensureRecentCallsColumn(
      "queue_entered_at",
      "ALTER TABLE recent_calls ADD COLUMN queue_entered_at datetime(6) NULL AFTER queue_position",
    );
    await this.ensureRecentCallsColumn(
      "queue_exited_at",
      "ALTER TABLE recent_calls ADD COLUMN queue_exited_at datetime(6) NULL AFTER queue_entered_at",
    );
    await this.ensureRecentCallsColumn(
      "queue_wait_seconds",
      "ALTER TABLE recent_calls ADD COLUMN queue_wait_seconds int NULL AFTER queue_exited_at",
    );
    await this.ensureRecentCallsColumn(
      "queue_callback_requested",
      "ALTER TABLE recent_calls ADD COLUMN queue_callback_requested tinyint(1) NOT NULL DEFAULT 0 AFTER queue_wait_seconds",
    );
    await this.ensureRecentCallsColumn(
      "voicemail_transcription",
      "ALTER TABLE recent_calls ADD COLUMN voicemail_transcription longtext NULL AFTER queue_callback_requested",
    );
    await this.ensureRecentCallsColumn(
      "ai_summary",
      "ALTER TABLE recent_calls ADD COLUMN ai_summary longtext NULL AFTER voicemail_transcription",
    );
    await this.ensureRecentCallsColumn(
      "ai_sentiment",
      "ALTER TABLE recent_calls ADD COLUMN ai_sentiment varchar(64) NULL AFTER ai_summary",
    );
    await this.ensureRecentCallsColumn(
      "ai_status",
      "ALTER TABLE recent_calls ADD COLUMN ai_status varchar(64) NULL AFTER ai_sentiment",
    );
    await this.ensureRecentCallsColumn(
      "ai_provider",
      "ALTER TABLE recent_calls ADD COLUMN ai_provider varchar(64) NULL AFTER ai_status",
    );
    await this.ensureRecentCallsColumn(
      "ai_model",
      "ALTER TABLE recent_calls ADD COLUMN ai_model varchar(128) NULL AFTER ai_provider",
    );
    await this.ensureRecentCallsColumn(
      "ai_enriched_at",
      "ALTER TABLE recent_calls ADD COLUMN ai_enriched_at datetime(6) NULL AFTER ai_model",
    );
