...
import {
  isOperationalAccessPreActivationRoute,
  isOperationalAccessPublicRoute,
  requiresOperationalAccess,
} from "../auth/operational-access.policy";
...
function requireMySqlOptions(): MysqlConnectionOptions {
  ...
  if (options.type !== "mysql" && options.type !== "mariadb") {
    throw new Error("Operational access isolation smoke currently supports MySQL only.");
  }
  return {
...
async function seedOrganization(dataSource: DataSource, token: string) {
  ...
  const organization = await orgRepo.save(
    orgRepo.create({
      name: `Operational Access Org ${token}`,
      slug: `ops-access-${token}`,
      is_active: true,
    }),
  );
...
async function runPolicyCases(summary: SmokeSummary) {
  ...
  await expectPass(summary, "O2 — auth and pre-activation billing routes bypass operational access", async () => {
    assert.equal(requiresOperationalAccess("/api/auth/login"), false);
    assert.equal(isOperationalAccessPreActivationRoute("/api/billing/summary"), true);
    assert.equal(requiresOperationalAccess("/api/billing/summary"), false);
    return {
      auth: "/api/auth/login",
...
async function runDbCases(summary: SmokeSummary, dataSource: DataSource, seed: { organizationId: string; billingAccountId: string }) {
  const harness = buildBillingSmokeHarness(dataSource);

  await expectPass(summary, "O4 — unpaid workspace denied operational access", async () => {
    const eligible = await harness.authService.isOrganizationOperationallyEligible(seed.organizationId);
    if (eligible) throw new Error("Unpaid workspace must be denied operational access.");
    return { eligible };
  });
  ...
  await expectPass(summary, "O6 — controlled access allowlist unlocks unpaid workspace", async () => {
    const unpaidSeed = await seedOrganization(dataSource, randomUUID().slice(0, 8));
    const grantRepo = dataSource.getRepository(ControlledAccessGrantEntity);
    const now = new Date();
    await grantRepo.save(
      grantRepo.create({
        organization_id: unpaidSeed.organizationId,
        grant_type: "pilot",
        reason_code: "operational_access_isolation_smoke",
    ...
    const eligible = await harness.authService.isOrganizationOperationallyEligible(unpaidSeed.organizationId);
    if (!eligible) throw new Error("Controlled access grant must unlock operational access.");
    return { eligible };
  });
}
...
async function main() {
  ...
  const databaseName = process.env.DB_SMOKE_DATABASE?.trim() || `wizfield_ops_access_${Date.now()}`;
  ...
  try {
    ...
    const seed = await seedOrganization(dataSource, randomUUID().slice(0, 8));
    summary.phases.seeding = "PASS";

    await runPolicyCases(summary);
    await runDbCases(summary, dataSource, seed);
