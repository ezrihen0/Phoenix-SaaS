...
export default async function BillingSuccessPage({ searchParams }: BillingSuccessPageContext) {
  ...
  return (
    <MarketingFunnelShell>
      <main className="mx-auto max-w-2xl px-6 py-16 text-white lg:px-10">
        ...
        {sessionId && process.env.NODE_ENV === "development" ? (
          <div className="mt-6 rounded-[20px] border border-white/10 bg-black/30 px-4 py-4">
            <p className="text-xs uppercase tracking-[0.18em] text-white/45">Stripe Checkout session id (development only)</p>
            <p className="mt-2 break-all font-mono text-xs text-white/80">{sessionId}</p>
          </div>
        ) : null}
      </main>
    </MarketingFunnelShell>
