...
type CreateInvoiceChooserPageContext = {
...
export default async function CreateInvoiceChooserPage({ searchParams }: CreateInvoiceChooserPageContext) {
  await requireServerPermission("/invoices/create", "invoices.manage");
                  ...
                  {customers.length ? customers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-[color:var(--cmp-surface-panel)]">
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link
                          href={`/invoices/create?customerId=${encodeURIComponent(customer.id)}`}
                ...
                {relatedJobs.length ? relatedJobs.map((job) => {
                  ...
                  return (
                    <tr key={job.id} className="hover:bg-[color:var(--cmp-surface-panel)]">
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link
                          href={jobInvoice ? `/invoices/${jobInvoice.id}` : `/invoices/create/${job.id}`}
                          className="theme-btn-secondary inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs uppercase tracking-[0.18em] transition"
                        >
