...
import FinanceJobBootstrapPanel from "@/lib/crm/finance-job-bootstrap-panel";

type SearchParam = string | string[] | undefined;

type NewEstimatePageContext = {
...
function formatEstimateLifecycle(status: EstimateLifecycleStatus) {
  if (status === "void") {
    return "Voided";
  }
...
function estimateLifecycleClass(status: EstimateLifecycleStatus) {
...
export default async function NewEstimatePage({ searchParams }: NewEstimatePageContext) {
  await requireServerPermission("/estimates/create", "estimates.manage");
        ...
        <div className="mx-auto max-w-6xl px-6 py-10 lg:px-10">
          <section className="theme-surface-modal rounded-[36px] p-7 sm:p-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-[0.36em] text-[color:var(--sem-accent-primary)]">Create Estimate</p>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-[color:var(--sem-text-secondary)] sm:text-base">
                  Select a customer, then choose the job where you want to create or update an estimate.
                </p>
              </div>
              <Link
                href="/estimates"
                className="inline-flex items-center gap-2 rounded-full border border-[color:var(--cmp-border-subtle)] px-4 py-2 text-sm text-[color:var(--sem-text-secondary)] transition hover:border-[color:var(--cmp-border-subtle)] hover:text-[color:var(--sem-text-primary)]"
              >
          ...
          <section className="theme-surface-modal mt-6 rounded-[32px] p-6">
            ...
            {loadError ? (
              <div className="mt-5 rounded-[22px] border theme-alert-error px-4 py-3 text-sm">
                {loadError}
              </div>
            ) : null}
                  ...
                  {customers.length ? customers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-[color:var(--cmp-surface-panel)]">
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link
                          href={`/estimates/create?customerId=${encodeURIComponent(customer.id)}`}
  ...
  if (detail) {
    try {
      customerEstimates = await serverApiFetch<EstimateListItem[]>(`/api/estimates?customerId=${encodeURIComponent(customerId)}`);
    } catch (error) {
      loadError = error instanceof Error ? error.message : "Customer estimates could not be loaded.";
    }
  }

  const customer = detail?.customer ?? null;
  const relatedJobs = detail?.relatedJobs ?? [];
  const estimateByJobId = new Map(customerEstimates.map((estimate) => [estimate.job_id, estimate]));
  ...
  return (
    <main className="min-h-screen bg-[color:var(--cmp-surface-canvas)] text-[color:var(--sem-text-primary)]">
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-10">
        <section className="theme-surface-modal rounded-[36px] p-7 sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[color:var(--sem-text-secondary)] sm:text-base">
                Choose a customer job, then open the quote section to create or update the estimate document.
              </p>
            </div>
          ...
          {customer ? (
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              ...
              <article className="rounded-[24px] border border-[color:var(--cmp-border-subtle)] bg-[color:var(--cmp-surface-panel)] p-5">
                <p className="text-[11px] uppercase tracking-[0.28em] text-[color:var(--sem-text-muted)]">Estimates</p>
                <p className="mt-3 text-3xl font-semibold text-[color:var(--sem-text-primary)]">{customerEstimates.length}</p>
              </article>
              <article className="rounded-[24px] border border-[color:var(--cmp-border-subtle)] bg-[color:var(--cmp-surface-panel)] p-5">
                <p className="text-[11px] uppercase tracking-[0.28em] text-[color:var(--sem-text-muted)]">Contact</p>
                <p className="mt-3 text-sm text-[color:var(--sem-text-primary)]">{customer.phone}</p>
                <p className="mt-2 text-xs text-[color:var(--sem-text-secondary)]">{customer.email ?? "No email on file"}</p>
              </article>
            </div>
          ) : null}
        </section>
        ...
        <section className="theme-surface-modal mt-6 rounded-[32px] p-6">
          {loadError ? (
            <div className="rounded-[22px] border theme-alert-error px-4 py-3 text-sm">
              {loadError}
            </div>
          ) : null}
            ...
            <table className="crm-table text-left text-sm">
              ...
              <tbody className="bg-[color:var(--cmp-surface-panel)]">
                {relatedJobs.length ? relatedJobs.map((job) => {
                  const jobEstimate = estimateByJobId.get(job.id);
                  ...
                  return (
                    <tr key={job.id} className="hover:bg-[color:var(--cmp-surface-panel)]">
                      ...
                      <td className="px-5 py-4">
                        <span className="theme-badge inline-flex rounded-full border bg-[color:var(--cmp-surface-panel)] px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-[color:var(--sem-text-secondary)]">
                      ...
                      <td className="px-5 py-4">
                        {jobEstimate ? (
                          <div className="space-y-2">
                            <p className="text-xs text-[color:var(--sem-text-secondary)]">{jobEstimate.document_number}</p>
                            <span className={estimateLifecycleClass(jobEstimate.lifecycle_status)}>
                              {formatEstimateLifecycle(jobEstimate.lifecycle_status)}
                            </span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-2 text-[color:var(--sem-text-muted)]">
                            <FileText className="h-4 w-4" />
                            <span className="text-xs">No estimate yet</span>
                          </div>
                        )}
                      </td>
                      <td className="px-5 py-4 text-right">
                        <Link
                          href={jobEstimate ? `/estimates/${jobEstimate.id}` : `/estimates/create/${job.id}`}
                          className="theme-btn-secondary inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs uppercase tracking-[0.18em] transition"
                        >
                          {jobEstimate ? "Open Estimate" : "Create Estimate"}
                          <ArrowRight className="h-3.5 w-3.5" />
                        </Link>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={5} className="px-5 py-10 text-center text-sm text-[color:var(--sem-text-muted)]">
                      No jobs found for this customer. Create a minimal job below or use full job intake.
                      {customer ? (
                        <FinanceJobBootstrapPanel
