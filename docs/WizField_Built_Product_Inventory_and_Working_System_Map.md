...
- **Stripe live activation is DEFERRED** (owner decision)—not treated as product-map failure.
...
| **Immediate stop conditions** | Do not treat untracked AI chat files as released truth until committed/reviewed; do not assume i18n parity is clean (`i18n:check` **FAIL**); do not run live Stripe/Telnyx/SMS/email in mapping mode |
...
| **/signup** | [`frontend/app/signup/page.tsx`](frontend/app/signup/page.tsx) | `POST /api/auth/register`, org creation | users, profiles, memberships, organizations | `client-auth` | Public | bootstrap admin env | P0 walkthrough **PASS** | **VERIFIED** (API) | P1 | Routes to `/pricing` without Stripe live |
...
| **/settings** | [`frontend/app/settings/page.tsx`](frontend/app/settings/page.tsx), **untracked** `ai-usage-panel.tsx` | [`settings.controller.ts`](backend/src/settings/settings.controller.ts), auth staff/orgs, billing summary | `organization_settings`, memberships, billing tables | `client-auth`, `client-billing`, `server-fetch` | Owner/admin slices | Stripe env for checkout | build | **PARTIAL** | P1 | Billing UI redirects from `/billing`; AI usage panel untracked |
...
| **/pricing** | [`(marketing)/pricing`](frontend/app/(marketing)/pricing/page.tsx) | billing checkout-session (Stripe) | billing_accounts, organization_billing | `client-billing`, `plan-display` | Public + session destination | Stripe price IDs | build | **DEFERRED** (live checkout) | P4 | Engineering checkout exists; **live Stripe owner activation deferred** |
| **/billing/success** | [`frontend/app/billing/success/page.tsx`](frontend/app/billing/success/page.tsx) | Reads session; **does not activate billing** | organization_billing | `client-auth`, server session | Authenticated | — | build | **VERIFIED** (engineering intent) | P4 | Processing-only copy; webhook is authority |
